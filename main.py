...
bot = CustomClient(
   ...
   name="My_really_cool_bot_bot",
)

@bot.on_message(filters=filters.command('start') | button_filter(buttons.back_button))
async def start_bot(client: CustomClient, message: Message):
...
async def help_commands(client: CustomClient, message: Message):
...
async def settings_command(client: CustomClient, message: Message):
    await message.reply("⚙️Настройки", reply_markup=keyboards.settings_keyboard)
